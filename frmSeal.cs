    ...
    public partial class frmSeal : Form
    {
        ...
        //********************************************
            ...
            //===============
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    PopulateMaterial(mcmbMatBase[i], "fldBase");
                    PopulateMaterial(mcmbMatLining[i], "fldLining");
                } 
            }
            ...
            private void PopulateMaterial(ComboBox cmbBox_In, string strFldName_In)
            //=========================================================================
            {
                //.....Retrieve Lining and Base Material. 
                string pstrWHERE = " WHERE " + strFldName_In + " = 1";
                //int pRecCountBaseMat = modMain.gDB.PopulateCmbBox(cmbBox_In, "tblData_Mat", "fldType", pstrWHERE, true);  //BG 02AUG11
                int pRecCountBaseMat = modMain.gDB.PopulateCmbBox(cmbBox_In, "tblData_Mat", "fldName", pstrWHERE, true);
                if (pRecCountBaseMat > 0) cmbBox_In.SelectedIndex = 0;
        ...
        //******************************
            ...
            //===========================
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    {
                        //mEndSeal[i] = (clsSeal)modMain.gEndSeal[i].Clone();
            ...
            //===============================================
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    mtxtDBore[i].ReadOnly = !pEnable_In;
                }
                ...
                {
                    //....Type
                    mcmbSealType[i].Enabled = pEnable_In;

                    //....Blade 
                    mcmbBladeCount[i].Enabled = pEnable_In;
                    mtxtBladeT[i].ReadOnly = !pEnable_In;
                    mcmbBladeAngleTaper[i].Enabled = pEnable_In;

                    //....Materials.
                    mcmbMatBase[i].Enabled = pEnable_In;
                    mcmbMatLining[i].Enabled = pEnable_In;
                    mtxtMatLiningT[i].Enabled = pEnable_In;
                }                
            }
            ...
            //========================
            {
                for (int i = 0; i < 2; i++)
                {
                    if (modMain.gProject.Product.EndConfig[i].Type == clsEndConfig.eType.Seal)
                    {
                       ...
                       int k =0;
                        ...
                        for ( int j = 0; j < 2; j++, k++)
                        {
                            mtxtDBore[k].Text = modMain.ConvDoubleToStr(mEndSeal[i].DBore_Range[j], "#0.0000");
                        }
                        ...
                        //  -----
                            ...
                            if(mEndSeal[i].Blade.Count !=0)
                            {
                                ...
                                mtxtBladeT[i].Text =modMain.ConvDoubleToStr(mEndSeal[i].Blade.T,"#0.000");                                    
                        ...
                        //  --------
                           ...
                           mtxtMatLiningT[i].Text = modMain.ConvDoubleToStr(mEndSeal[i].Mat_LiningT, "");
                ...
                //{
                //   //....Seal Type.
                //    cmbSealType_Front.Text = mEndSeal[0].Design.ToString();

                //   //  Bore Dia
                //   //  ---------
                //       for (int i = 0; i < 2; i++)                       
                //        mtxtDBore[i].Text = modMain.ConvDoubleToStr(mEndSeal[0].DBore_Range[i], "#0.0000");

                //   //  Blade
                //   //  -----                   
                //       SetTxtForeColor(mEndSeal[0].Blade.T, txtBlade_T_Front, mEndSeal[0].Blade.DESIGN_BLADE_THICK);

                //    //....Count.
                        
                //        cmbBlade_Count_Front.Text = modMain.ConvIntToStr(((clsSeal)modMain.gProject.Product.EndConfig[0]).Blade.Count);

                //   //....Blade Taper Angel.
                //   //cmbBlade_AngTaper_Front.Text = modMain.ConvDoubleToStr(modMain.gEndSeal[0].Blade.AngTaper, "");
                //   cmbBlade_AngTaper_Front.Text = modMain.ConvDoubleToStr(((clsSeal)modMain.gProject.Product.EndConfig[0]).Blade.AngTaper, "");

                //   //  Material
                //   //  --------
                //   //cmbMat_Base_Front.Text = modMain.gEndSeal[0].Mat.Base;
                //   //cmbMat_Lining_Front.Text = modMain.gEndSeal[0].Mat.Lining.Lining;

                //   cmbMat_Base_Front.Text = ((clsSeal)modMain.gProject.Product.EndConfig[0]).Mat.Base;
                //   cmbMat_Lining_Front.Text = ((clsSeal)modMain.gProject.Product.EndConfig[0]).Mat.Lining;

                //   //txtMat_LiningT_Front.Text = modMain.ConvDoubleToStr(modMain.gEndSeal[0].Mat_LiningT, "");
                //   txtMat_LiningT_Front.Text = modMain.ConvDoubleToStr(((clsSeal)modMain.gProject.Product.EndConfig[0]).Mat_LiningT, "");
                //   SetMatLiningT(cmbMat_Lining_Front.Text);
                //   //SetThick(modMain.gEndSeal[0].Mat_LiningT, txtMat_LiningT_Front, mcLiningThickness);
                //   SetTxtForeColor(((clsSeal)modMain.gProject.Product.EndConfig[0]).Mat_LiningT, txtMat_LiningT_Front, mcLiningThickness);       
                    
                //}

                //if (modMain.gProject.Product.EndConfig[1].Type == clsEndConfig.eType.Seal)
                //{
                //    //cmbSealType_Back.Text = modMain.gEndSeal[1].Type.ToString();
                //    cmbSealType_Back.Text = modMain.gProject.Product.EndConfig[1].Type.ToString();

                //    //  Bore Dia
                //    //  ---------
                //    for (int i = 0, j = 2; i < 2; i++, j++)
                //        //mtxtBoreD[j].Text = modMain.ConvDoubleToStr(modMain.gEndSeal[1].BoreD_Range[i], "#0.0000");
                //        mtxtDBore[i].Text = modMain.ConvDoubleToStr(modMain.gProject.Product.EndConfig[1].DBore_Range[i], "#0.0000");

                //    //  Blade
                //    //  -----
                //    //SetThick(modMain.gEndSeal[1].Blade.T, txtBlade_T_Back, mcBladeThickness);
                //    SetTxtForeColor(((clsSeal)modMain.gProject.Product.EndConfig[1]).Blade.T, txtBlade_T_Back, mcBladeThickness);

                //    //....Blade Count.
                //    //cmbBlade_Count_Back.Text = modMain.ConvIntToStr(modMain.gEndSeal[1].Blade.Count);
                //    cmbBlade_Count_Back.Text = modMain.ConvIntToStr(((clsSeal)modMain.gProject.Product.EndConfig[1]).Blade.Count);

                //    //....Blade Taper Angel.
                //    //cmbBlade_AngTaper_Back.Text = modMain.ConvDoubleToStr(modMain.gEndSeal[1].Blade.Count, "");
                //    cmbBlade_AngTaper_Back.Text = modMain.ConvDoubleToStr(((clsSeal)modMain.gProject.Product.EndConfig[1]).Blade.AngTaper, "");

                //    //  Material
                //    //  --------
                //    //cmbMat_Base_Back.Text = modMain.gEndSeal[1].Mat.Base;
                //    //cmbMat_Lining_Back.Text = modMain.gEndSeal[1].Mat.Lining.Lining;

                //    cmbMat_Base_Back.Text = ((clsSeal)modMain.gProject.Product.EndConfig[1]).Mat.Base;
                //    cmbMat_Lining_Back.Text = ((clsSeal)modMain.gProject.Product.EndConfig[1]).Mat.Lining;

                //    //txtMat_LiningT_Back.Text = modMain.ConvDoubleToStr(modMain.gEndSeal[1].Mat_LiningT, "");
                //    txtMat_LiningT_Back.Text = modMain.ConvDoubleToStr(((clsSeal)modMain.gProject.Product.EndConfig[1]).Mat_LiningT, "");
                //    SetMatLiningT(cmbMat_Lining_Back.Text);

                //    //SetThick(modMain.gEndSeal[1].Mat_LiningT, txtMat_LiningT_Back, mcLiningThickness);  
                //    SetTxtForeColor(((clsSeal)modMain.gProject.Product.EndConfig[1]).Mat_LiningT, txtMat_LiningT_Back, mcLiningThickness);  
                //}
        ...
        #region" CONTROL EVENT RELATED ROUTINE:"
            ...
            //-------------------------------
                ...
                //private void txtBlade_T_MouseDown(object sender, MouseEventArgs e)  
                ////================================================================
                //{
                //    TextBox pTxtBox = (TextBox)sender;
