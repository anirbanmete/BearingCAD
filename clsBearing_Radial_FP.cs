                   ...
                   //---------------------------------------
                    ...
                    //---------------------------                     
                    {
                        ...
                        pApprox = 0.25*(mMount.DFit_EndConfig(0) + DSet());             //....Always from the Front face. 
                                                                                        //........Fixture is used on the Front face.
 
                        string pstrFIELDS  = "fldB_Hdist_Nom";
                        string pstrFROM    = " FROM tblManf_Fixture_Bearing_Splitting ";
                        string pstrORDERBY = " ORDER BY fldB_Hdist_Nom ASC";
                        string pstrSQL     = "SELECT " + pstrFIELDS + pstrFROM + pstrORDERBY;
                        ...
                        while (pDR.Read())
                        {  
                            if (mDB.CheckDBDouble(pDR, pstrFIELDS) > pApprox)
                            {
                                pLoc_Fixture_B = mDB.CheckDBDouble(pDR, pstrFIELDS);
                                break;                               
                            }
                        }
                    ...
                    //----------------------------            
                    {
                        ...
                        pstrTblName = "tblDesign_Lining";
                        pstrFIELDS  = "fldT";
                        pstrFROM    = " FROM " + pstrTblName;
                        pstrWHERE = " WHERE fldDSet_LLim <= " + DSet() +
                                      " AND fldDSet_ULim >= " + DSet();
                        ...
                        if (pDR.Read())
                        {
                            pT = Convert.ToDouble(pDR[pstrFIELDS]);
                        }
                        pDR.Close();
                        pConnection.Close();
